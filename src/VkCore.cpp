...
void Core::setWindowPointer(GLFWwindow* window) {
    pWindow = window;
}
...
VkSurfaceFormatKHR Core::choooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& availableFormats) {
...
bool Core::checkDeviceExtensionsSupport(VkPhysicalDevice device) {
...
VkShaderModule Core::createShaderModule(const std::vector<char>& code) {
    ...
    if (vkCreateShaderModule(device, &createInfo, nullptr, &shaderModule) != VK_SUCCESS)
        Console::PrintError("Failed to create shader Module!");
...
void Core::recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
    ...
    if (vkBeginCommandBuffer(commandBuffer, &beginInfo) != VK_SUCCESS)
        Console::PrintError("Failed to begin recording command buffer!");
...
uint32_t Core::findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties) {
    ...
    Console::PrintError("Failed to find suitable memory type!");
}
...
void Core::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory) {
    ...
    if (vkCreateBuffer(device, &bufferInfo, nullptr, &buffer) != VK_SUCCESS)
        Console::PrintError("Failed to create buffer!");
    ...
    if (vkAllocateMemory(device, &allocInfo, nullptr, &bufferMemory) != VK_SUCCESS)
        Console::PrintError("failed to allocate buffer memory!");
...
void Core::updateUniformBuffer(uint32_t currentImage) {
...
void Core::createSwapChain() {
    ...
    if (vkCreateSwapchainKHR(device, &createInfo, nullptr, &swapChain) != VK_SUCCESS)
        Console::PrintError("Failed to create Swap chain!");
...
void Core::createImageViews() {
    ...
    for (size_t i = 0; i < swapChainImages.size(); i++) {
        ...
        if (vkCreateImageView(device, &createInfo, nullptr, &swapChainImageViews[i]) != VK_SUCCESS)
            Console::PrintError("Could not create Image Views");
    }
}
...
void Core::createRenderPass() {
    ...
    if (vkCreateRenderPass(device, &renderPassInfo, nullptr, &renderPass) != VK_SUCCESS)
        Console::PrintError("Failed to create render pass!");
}
...
void Core::createDescriptorSetLayout() {
    ...
    if (vkCreateDescriptorSetLayout(device, &layoutInfo, nullptr, &descriptorSetLayout) != VK_SUCCESS)
        Console::PrintError("Failed to create Descriptor Set Layout!");
}

void Core::createGraphicsPipeline() {
    const std::vector<char> vertShaderCode = IO::readBinaryFile("triangle_vert.spv");
    const std::vector<char> fragShaderCode = IO::readBinaryFile("triangle_frag.spv");

    if (vertShaderCode.empty() || fragShaderCode.empty())
        Console::PrintError("Could not load shaders");
    ...
    if (vkCreatePipelineLayout(device, &pipelineLayoutInfo, nullptr, &pipelineLayout) != VK_SUCCESS)
        Console::PrintError("Failed to create pipeline layout!");
    ...
    if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &graphicsPipeline) != VK_SUCCESS)
        Console::PrintError("Failed to create graphics pipeline!");

    if (isDebug) Console::PrintInfo("Shaders Loaded!");
...
void Core::createFramebuffers() {
    ...
    for (size_t i = 0; i < swapChainImageViews.size(); i++) {
        ...
        if (vkCreateFramebuffer(device, &framebufferInfo, nullptr, &swapChainFramebuffers[i]) != VK_SUCCESS)
            Console::PrintError("Failed to create framebuffer!");
    }
}
...
void Core::createUniformBuffers() {
...
void Core::createDescriptorPool() {
    ...
    if (vkCreateDescriptorPool(device, &poolInfo, nullptr, &descriptorPool) != VK_SUCCESS)
        Console::PrintError("Failed to create Descriptor Pool!");
}
...
void Core::createDescriptorSets() {
    ...
    if (vkAllocateDescriptorSets(device, &allocInfo, descriptorSets.data()) != VK_SUCCESS)
        Console::PrintError("Failed to allocate descriptor sets!");
...
void Core::createCommandPool() {
    ...
    if (vkCreateCommandPool(device, &poolInfo, nullptr, &commandPool) != VK_SUCCESS)
        Console::PrintError("Failed to allocate command buffer!");
}
...
void Core::createCommandBuffer() {
    ...
    if (vkAllocateCommandBuffers(device, &allocInfo, commandBuffers.data()) != VK_SUCCESS)
        Console::PrintError("Failed to allocate command buffers!");
}
...
void Core::createSyncObjects() {
    ...
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        if (vkCreateSemaphore(device, &semaphoreInfo, nullptr, &imageAvailableSemaphores[i]) != VK_SUCCESS ||
            ...
            Console::PrintError("Failed to create semaphores!");
    }
}

void Core::drawFrame() {
    // Wait for GPU to draw
...
void Core::cleanup() {
    ...
    if (isDebug) Console::PrintInfo("Destroying Uniform Buffer!");
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        vkDestroyBuffer(device, uniformBuffers[i], nullptr);
        vkFreeMemory(device, uniformBuffersMemory[i], nullptr);
    }

    if (isDebug) Console::PrintInfo("Destroying Descriptor Pool!");
    vkDestroyDescriptorPool(device, descriptorPool, nullptr);

    if (isDebug) Console::PrintInfo("Destroying Descriptor Set Layout!");
    vkDestroyDescriptorSetLayout(device, descriptorSetLayout, nullptr);

    if (isDebug) Console::PrintInfo("Destroying buffers!");
    ...
    if (isDebug) Console::PrintInfo("Destroying Sync Objects!");
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
    ...
    if (isDebug) Console::PrintInfo("Destroying Pipeline Layout!");
